...
def add_users():
    try :
        ...
        user = models.user.User(name=name ,email=email ,phone=phone ,password=password)
...
def delete_users():
    try:
        ...
        id = input("Enter row id if you delet: ")
        if id :
            is_ok = input("are you oky to delet , defult n :(y/n)")
            if is_ok in ['ok','yes','y',"Y"] :
                return db.delete_user(table_name=table_name, id=id)
            else :
                return "delet concel"
        else :
...
def update_users():
    try :
        ...
        return db.update_user(query=condition,table_name=table_name, name=new_name if new_name else None, email=new_email if new_email else None, phone=new_phone if new_phone else None, password=new_password if new_password else None)
    except Exception as e :
...
def delet_table():
    try :
        return db.drop_table(table_name=input("table name :"))
    except Exception as e :
        return f"error: {e}"
